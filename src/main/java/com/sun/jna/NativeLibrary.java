...
/**
 * Provides management of native library resources. One instance of this class
 * corresponds to a single loaded native library. May also be used to map to the
 * current process (see {@link NativeLibrary#getProcess()}).
 * <p>
 * <a name=library_search_paths></a> <b>Library Search Paths</b> A search for a
 * given library will scan the following locations:
 * <ol>
 ...
 * <code>/Library/Frameworks</code>, and <code>/System/Library/Frameworks</code>
 * will be searched for a framework with a name corresponding to that requested.
 * Absolute paths to frameworks are also accepted, either ending at the
 * framework name (sans ".framework") or the full path to the framework shared
 * library (e.g. CoreServices.framework/CoreServices).
 * <li>Context class loader classpath. Deployed native libraries may be
 * installed on the classpath under <code>${os-prefix}/LIBRARY_FILENAME</code>,
 * where <code>${os-prefix}</code> is the OS/Arch prefix returned by
 * {@link Platform#getNativeLibraryResourcePrefix()}. If bundled in a jar file,
 * the resource will be extracted to <code>jna.tmpdir</code> for loading, and
 * later removed (but only if <code>jna.nounpack</code> is false or not set).
 * </ol>
 * You may set the system property <code>jna.debug_load=true</code> to make JNA
...
public class NativeLibrary
{
    ...
    private static String functionKey(final String name,
                                      final int flags,
    ...
    private NativeLibrary(final String libraryName,
                          ...
                          final Map options)
    {
        ...
        final int callingConvention = option instanceof Number
        ...
        if (Platform.isWindows() && "kernel32".equals(this.libraryName.toLowerCase())) {
            synchronized (functions) {
                final Function f = new Function(this, "GetLastError", Function.ALT_CONVENTION, encoding)
                {
    ...
    {
        final Object opt = options.get(Library.OPTION_OPEN_FLAGS);
    ...
    private static NativeLibrary loadLibrary(final String libraryName,
                                             final Map options)
    {
        ...
        try {
            ...
            if (Platform.isAndroid()) {
                ...
                //
                // Failed to load the library normally - try to match
                // libfoo.so.*
                //
                if (Native.DEBUG_LOAD) {
            ...
            if (handle == 0) {
                try {
                    final File embedded = Native.extractFromResourcePath(libraryName,
    ...
    private String getLibraryName(final String libraryName)
    {
        ...
        final String BASE = "---";
        final String template = mapSharedLibraryName(BASE);
    ...
    /**
     * Returns an instance of NativeLibrary for the specified name. The library
     * is loaded if not already loaded. If already loaded, the existing instance
     * is returned.
     * <p>
     * More than one name may map to the same NativeLibrary instance; only a
     * single instance will be provided for any given unique file path.
     *
     * @param libraryName The library name to load. This can be short form (e.g.
     *            "c"), an explicit version (e.g. "libc.so.6"), or the full path
     *            to the library (e.g. "/lib/libc.so.6").
     */
     ...
     *            included somewhere in the classpath, either bundled in a jar
     *            file or as a plain file within the classpath.
     */
    public static final NativeLibrary getInstance(final String libraryName,
     ...
     *
     * @param libraryName The library name to load. This can be short form (e.g.
     *            "c"), an explicit version (e.g. "libc.so.6" or
     *            "QuickTime.framework/Versions/Current/QuickTime"), or the full
     *            (absolute) path to the library (e.g. "/lib/libc.so.6").
     * @param options native library options for the given library (see
     *            {@link Library}).
     */
     ...
     * other mechanism, without having to reference or even know the exact name
     ...
     * paths. This is similar to setting <code>jna.library.path</code>, but only
     * extends the search path for a single library.
     *
     * @param libraryName The name of the library to use the path for
     * @param path The path to use when trying to load the library
     */
    public static final void addSearchPath(final String libraryName,
                                           final String path)
    {
    ...
    /**
     * Create a new {@link Function} that is linked with a native function that
     * follows the NativeLibrary's calling convention.
     *
     * <p>
     * The allocated instance represents a pointer to the named native function
     * from the library.
     *
     ...
     * from the library.
     *
     * @param name Name of the native function to be linked with. Uses a
     *            function mapper option if one was provided to transform the
     *            name.
    ...
    Function getFunction(String name,
                         final Method method)
    {
        final FunctionMapper mapper = (FunctionMapper) options.get(Library.OPTION_FUNCTION_MAPPER);
        if (mapper != null) {
    ...
    /**
     * Create a new {@link Function} that is linked with a native function that
     * follows a given calling flags.
     *
     * @param functionName Name of the native function to be linked with
     * @param callFlags Flags affecting the function invocation
     * @throws UnsatisfiedLinkError if the function is not found
     */
    public Function getFunction(final String functionName,
                                final int callFlags)
     ...
     * @param functionName Name of the native function to be linked with
     * @param callFlags Flags affecting the function invocation
     * @param encoding Encoding to use to convert between Java and native
     *            strings.
     * @throws UnsatisfiedLinkError if the function is not found
     */
    public Function getFunction(final String functionName,
    ...
    /**
     * Returns the file on disk corresponding to this NativeLibrary instance. If
     * this NativeLibrary represents the current process, this function will
     * return null.
     */
    public File getFile()
    {
    ...
    private static String findLibraryPath(final String libName,
                                          final List searchPath)
    {
    ...
    /**
     * Similar to {@link System#mapLibraryName}, except that it maps to standard
     * shared library formats rather than specifically JNI formats.
     *
     ...
     * where /usr/lib/libc.so does not exist, or it is not a valid symlink to a
     * versioned file (e.g. /lib/libc.so.6).
     */
    static String matchLibrary(final String libName,
                               List searchPath)
    {
    ...
    private static ArrayList<String> getLinuxLdPaths()
    {
        ...
        try {
            ...
            while ((buffer = reader.readLine()) != null) {
                ...
                final int endPath = buffer.lastIndexOf('/');
